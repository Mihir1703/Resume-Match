...
def cf_score(username):
    ...
    json_response = json.loads(requests.get(f"https://codeforces.com/api/user.rating?handle={username}").content)
    contest_count = len(json_response['result'])
    json_response = json.loads(requests.get(f"https://codeforces.com/api/user.status?handle={username}").content)
